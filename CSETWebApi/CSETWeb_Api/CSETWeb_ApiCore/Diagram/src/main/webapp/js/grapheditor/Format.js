...
PropertiesPanel.prototype.addProperties = function (container) {
    ...
    if (cell.isVertex()) {
        ...
        else if (cell.isBlueConnector()) {
            propertySet = diagramElementProperties().connectorProperties;
        }
        else {
        ...
        if (pp.type === 'select') {
            ...
            if (!!pp.values) {
                for (var j = 0; j < pp.values.length; j++) {
                    ...
                    option.setAttribute('value', pp.values[j]);
                    mxUtils.write(option, pp.values[j]);
                }

                // set the existing value
                for (var o = 0; o < ctl.options.length; o++) {
                    if (ctl.options[o].value === cell.getCsetAttribute(pp.attributeName)) {
                        ctl.selectedIndex = o;
                    }
                }
