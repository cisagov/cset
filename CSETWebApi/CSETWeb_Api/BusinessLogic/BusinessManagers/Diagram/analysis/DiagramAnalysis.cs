...
namespace CSETWeb_Api.BusinessManagers.Diagram.Analysis
{
    internal class DiagramAnalysis
    {
        ...
        private NetworkNode findNode(string id)
        {
            ...
            private List<INetworkAnalysisMessage> ListAnalysisMessages;
            ...
            {
                ListAnalysisMessages = new List<INetworkAnalysisMessage>();                
            }
            ...
            public void AnalyzeNetwork()
            {
                ...
                {
                    //("An exception occurred when doing analysis of the diagram.  ", ex);
                    //("Stack Trace of diagram analysis error." + ex.StackTrace);
                    //("There was an error while running analysis on the diagram. The analysis of the diagram may be inaccurate.");
                }
            }

            internal void GetDiagramAnalysisMessages()
            {
                RunAnalysis(false);
            }

            private void RunAnalysis(Boolean createAnalysisNodes)
            {
                stopWatch.Restart();
                ListAnalysisMessages.Clear();
                ...
                foreach (NetworkLink link in assessmentModel.NetworkModel.NetworkData.Links)
                {
                    if (link.IsVisible)
                    {
                        ILinkComponent headComponent = link.TargetComponent;
                        ILinkComponent tailComponent = link.SourceComponent;
                ...
                foreach (ILinkComponent component in assessmentModel.NetworkModel.NetworkData.DictionaryLinkComponents.Values)
                {
                ...
                stopWatch.Stop();
                Debug.WriteLine("Time Analysis: " + stopWatch.ElapsedMilliseconds);
            ...
            {
                if (NetworkContainer == null) //Doing this so can load diagram in reports
                    return;

                foreach (DiagramLayer layer in NetworkContainer.NetworkLayerManager.Layers)
                {
                    layer.ClearAnalyzeNodes();
                }

                foreach (NetworkAnalysisNode node in ListAnalysisNodes)
                {
                    NetworkContainer.RemoveItem(node);
                }
                ListAnalysisNodes.Clear();
            }

            private String rule7 = "Data flow between two distinct SAL zones via a unidirectional {0} must only flow from a higher SAL to a lower SAL. Example Control or SCADA network SAL High to Corporate Network SAL Low, but not SAL Low to SAL High.";

            private void CheckRule7(ILinkComponent component)
            {
                if (component.IsUnidirectional)
                {
                    SAL_LEVELS leftPosition = null;
                    SAL_LEVELS rightPosition = null;
                    List<NetworkLink> connectors = component.GetConnectors();
                    foreach (NetworkLink link in connectors)
                    {
                        if (link.SourceComponent == null || link.TargetComponent == null)
                            continue;

                        ILinkComponent otherComponent = GetOtherComponent(link, component);
            ...
            private void CheckRule34(ILinkComponent component)
            {
            ...
            private void CheckRule12(NetworkLink link, ILinkComponent headComponent, ILinkComponent tailComponent)
            {
            ...
            private List<EdgeNodeInfo> CheckFirewallUni(NetworkLink link, ILinkComponent component, HashSet<Guid> SetVisted = null)
            {
            ...
            private void CheckFirewallUniRecursive(ref List<EdgeNodeInfo> listNode, HashSet<Guid> visitedNodes, ILinkComponent component)
            {
                foreach (NetworkLink edge in component.GetConnectors())
                {
                    ILinkComponent otherComponent = GetOtherComponent(edge, component);
                    if (otherComponent == null)
                        continue;
            ...
            private void CheckRule2(ILinkComponent firewall)
            {
            ...
            private void CheckRule5(ILinkComponent component)
            {
            ...
            private void CheckRule6(HashSet<Guid> checkedLines, NetworkLink link, ILinkComponent headComponent, ILinkComponent tailComponent)
            {
            ...
            private void SetNodeMessage(ILinkComponent component, string text)
            {
                DiagramAnalysisNodeMessage messageNode;
                if (dictionaryNodeMessages.ContainsKey(component.ID))
                {
                    messageNode = dictionaryNodeMessages[component.ID];
                }
                else
                {
                    messageNode = new DiagramAnalysisNodeMessage(component);
                    dictionaryNodeMessages.Add(component.ID, messageNode);
                }
            ...
            private void AddMessage(NetworkLink link, String text)
            {
                ...
                {
                    ILinkComponent componentSource = link.Source as ILinkComponent;
                    ...
                    ILinkComponent componentTarget = link.Target as ILinkComponent;
            ...
            private List<EdgeNodeInfo> GetNodeEdges(ILinkComponent component, HashSet<String> typeNodes, HashSet<Guid> SetVisted = null)
            {
                SetVisted = SetVisted ?? new HashSet<Guid>();
                SetVisted.Add(component.ID);
                List<EdgeNodeInfo> listInfo = new List<EdgeNodeInfo>();
            ...
            private void GetNodeInfoRecursive(ref List<EdgeNodeInfo> listNode, HashSet<String> typeNodes, HashSet<Guid> visitedNodes, ILinkComponent component)
            {
            ...
            private bool CheckAddVisited(HashSet<Guid> visitedNodes, ILinkComponent otherComponent)
            {
            ...
            private ILinkComponent GetOtherComponent(NetworkLink link, ILinkComponent component)
            {
                ILinkComponent sourceComponent = link.SourceComponent;
                ILinkComponent targetComponent = link.TargetComponent;
