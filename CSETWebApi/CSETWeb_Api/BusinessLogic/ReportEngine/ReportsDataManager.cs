...
namespace CSETWeb_Api.BusinessLogic.ReportEngine
{
    public class ReportsDataManager : QuestionRequirementManager
    {
        ...
        /// 
        /// TODO QUESTION - Should this be using RelevantAnswers.GetAnswersForAssessment() instead?  
        ///                 Currently we arne't getting back any relevent component questions using RelevantAnswers.
        /// 
        /// TODO QUESTION - should this be filtering on questions matching the assessment's SAL?
        /// 
        /// TODO QUESTION - where does 'zone' come from?
        /// 
        /// </summary>
        /// <returns></returns>
        public List<ComponentQuestion> GetComponentQuestions()
        {
            ...
            using (var db = new CSET_Context())
            {
                var dbList = from a in db.Answer_Components
                             join ss in db.STANDARD_SELECTION on a.Assessment_Id equals ss.Assessment_Id
                             join usl in db.UNIVERSAL_SAL_LEVEL on ss.Selected_Sal_Level equals usl.Full_Name_Sal
                             join adc in db.ASSESSMENT_DIAGRAM_COMPONENTS on a.Assessment_Id equals adc.Assessment_Id
                             join q in db.NEW_QUESTION on a.Question_Or_Requirement_Id equals q.Question_Id
                             where a.Assessment_Id == _assessmentId
                                 && q.Universal_Sal_Level == usl.Universal_Sal_Level1
                             select new { a, adc, q };

                foreach (var component in dbList.OrderBy(x => x.adc.Diagram_Component_Type))
                {
                    l.Add(new ComponentQuestion
                    {
                        ComponentName = component.adc.label + " - " + component.adc.Diagram_Component_Type,
                        Question = component.q.Simple_Question,
                        Answer = component.a.Answer_Text,
                        Zone = "",  // TODO:  WHERE DO WE GET THIS?
                        SAL = component.q.Universal_Sal_Level
                    });
                }
            }
