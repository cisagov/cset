    ...
    internal class ConvertDatabase11100 : ConvertSqlDatabase
    {
        ...
        public override void Execute(SqlConnection conn)
        {
            try
            {
                ...
                RunFile(Path.Combine(this.applicationPath, @"VersionUpgrader\SQL\11013_to_11100_data3.sql"), conn);
                RunFile(Path.Combine(this.applicationPath, @"VersionUpgrader\SQL\11013_to_11100_data4.sql"), conn);
                this.UpgradeToVersionLocalDB(conn, myVersion);
            }
            catch (Exception e)
            {
                throw new DatabaseUpgradeException("Error in upgrading assessment version 11.0.1.3 to 11.1.0.0: " + e.Message);
            }
        }
    }
}
