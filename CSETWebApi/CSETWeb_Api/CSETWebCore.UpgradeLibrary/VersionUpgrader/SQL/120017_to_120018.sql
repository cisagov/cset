...
Script created by SQL Compare version 14.7.8.21163 from Red Gate Software Ltd at 1/19/2023 8:17:48 AM
...
BEGIN
	...
	select a.Assessment_Id, mq.Mat_Question_Id into #AvailableMatQuestions
		...
		join AVAILABLE_MATURITY_MODELS amm on amm.model_id = mq.Maturity_Model_Id
			...
			--and ((mq.Mat_Question_Id not in (select Id from @ParentMatIds) or )
	...
	select
		...
		from ASSESSMENTS a 
			...
			join USERS u on a.AssessmentCreatorId = u.UserId
			join ASSESSMENT_CONTACTS c on a.Assessment_Id = c.Assessment_Id and c.UserId = @User_Id
			where u.UserId = @User_Id and ans.Answer_Text != 'U'-- or (ans.Answer_Text = 'U' and ans.Free_Response_Answer is not null)
