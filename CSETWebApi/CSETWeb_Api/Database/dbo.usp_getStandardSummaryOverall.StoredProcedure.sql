USE [CSETWeb]
GO
/****** Object:  StoredProcedure [dbo].[usp_getStandardSummaryOverall]    Script Date: 11/14/2018 3:57:31 PM ******/
SET ANSI_NULLS ON
GO
...
begin	
		...
		SELECT a.Answer_Text, isnull(count(c.question_id),0) qc, SUM(count(c.question_id)) OVER() AS Total
...
begin 
		...
		SELECT a.Answer_Text, isnull(count(c.Requirement_Id),0) qc, SUM(count(c.Requirement_Id)) OVER() AS Total  
				FROM Answer_Questions a 
