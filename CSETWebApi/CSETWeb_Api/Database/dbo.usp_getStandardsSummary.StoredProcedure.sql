USE [CSETWeb]
GO
/****** Object:  StoredProcedure [dbo].[usp_getStandardsSummary]    Script Date: 11/14/2018 3:57:31 PM ******/
SET ANSI_NULLS ON
GO
...
begin	
		...
		SELECT ms.Short_Name, a.Answer_Text, isnull(count(c.question_id),0) qc, SUM(count(c.question_id)) OVER() AS Total
...
begin 
		select a.Answer_Full_Name, a.Answer_Text, m.Set_Name, 
			isnull(m.[qc], 0) as [qc], 
			isnull(m.[Total], 0) as [Total],
			isnull(cast(IsNull(Round((cast((qc) as float)/(isnull(nullif(Total,0),1)))*100,0),0) as int),0) as [Percent] 
		from answer_lookup a left join (
		SELECT s.Set_Name, a.Answer_Text, isnull(count(c.Requirement_Id),0) qc, SUM(count(c.Requirement_Id)) OVER() AS Total  
				...
				group by s.Set_Name, a.Answer_Text
	   ) m on a.Answer_Text = m.Answer_Text
end
END
