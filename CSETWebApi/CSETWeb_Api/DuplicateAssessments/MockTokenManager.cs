...
namespace DuplicateAssessments
{
    public class MockTokenManager : ITokenManager
    {
        ...
        public void Init(string tokenString)
        {
            ...
            //don't care if it is valid just march on
            // Convert to token 
        ...
        public string GenerateToken(int? userId, string accessKey, string tzOffset, int expSeconds, int? assessmentId, int? aggregationId, string scope)
        {
            ...
            var payload = new JwtPayload
            {
                ...
                { Constants.Token_AccessKey, accessKey },
        ...
        public bool IsTokenValid(string tokenString)
        {
            ...
            try
            {
                ...
                if (token.Payload[Constants.Token_UserId] != null)
                {
                    parms.IssuerSigningKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(GetSecret() + token.Payload[Constants.Token_UserId]));
                }
                else if (token.Payload[Constants.Token_AccessKey] != null)
                {
                    parms.IssuerSigningKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(GetSecret() + token.Payload[Constants.Token_AccessKey]));
                }
        ...
        {
            if (PayloadInt(Constants.Token_AssessmentId) != assessmentId)
            {
                var resp = new HttpResponseMessage(HttpStatusCode.Unauthorized)
                {
                    Content = new StringContent("User not authorized for assessment"),
        ...
        {
            return PayloadInt(Constants.Token_UserId);
        }
        ...
        {
            var userId = PayloadInt(Constants.Token_UserId);
            var user = _context.USERS.FirstOrDefault(x => x.UserId == userId);

            var accessKey = Payload(Constants.Token_AccessKey);
        ...
        {
            return PayloadInt(Constants.Token_AssessmentId) ?? 0;
        }
        ...
        /// <summary>
        /// Retrieves the JWT secret from the _context.  
        /// If the secret is not in the _context a new one is generated and persisted.
        ...
        {
            var uid = PayloadInt(Constants.Token_UserId);
            return uid;
        }
